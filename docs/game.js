/* Docs copy of game.js for GitHub Pages serving */

// (Content copied from root game.js)
(function () {
  if (window.__coding_clicker_loaded) return;
  window.__coding_clicker_loaded = true;
  ...
  function save() {
    safeSetItem(STORE_KEY, JSON.stringify(state));
  }
  ...
  wrap.innerHTML = `
    ...
    <div style="margin:10px 0"><div id="cc-click" class="bigclick" style="overflow-wrap:break-word">Click me (+<span id="cc-cpc">${state.cpc}</span>)</div></div>
  ...
  function render() {
    ...
    costCursorEl.textContent = cursorCost();
    costTutorEl.textContent = tutorCost();
  }

  function cursorCost() {
    return Math.floor(10 * Math.pow(1.15, state.upgrades.cursor));
  }
  function tutorCost() {
    return Math.floor(25 * Math.pow(1.17, state.upgrades.tutor));
  }
  ...
  buyTutorBtn.addEventListener("click", () => {
    const cost = tutorCost();
    if (state.coins >= cost) {
      state.coins -= cost;
      state.upgrades.tutor += 1;
      state.cpc += 1;
      save();
      render();
    } else {
  ...
  function cleanup() {
    const s = document.getElementById("codingclicker-style");
    if (s) s.remove();
    const o = document.getElementById(id);
    if (o) o.remove();
    clearInterval(tickInterval);
    try {
      delete window.__coding_clicker_loaded;
    } catch (e) {}
  }
  ...
  window.CodingGame.addCoins = (n) => {
    state.coins = (state.coins || 0) + Number(n || 0);
    save();
    render();
  };
